...
for case_study = 1:6
    ...
    for j = 1:2
        ...
        for k = 1:3
            ...
            load(filename)
            comput_time_point_base_10000 = datatable_all(21:25,5);
...
y = [ point_based_computational_time_max point_based_computational_time_sqrt Interval_Computational_Time];
h1 = bar(x,y);

%xlabel('Network', 'interpreter','latex','FontName','Times New Roman','FontSize',fs);
ylabel({'Computational'; 'time (second)'}, 'interpreter','latex','FontSize',fs);
l = cell(1,3);
%l{1}='Anlytical'; l{2}='Point-based-max'; l{3}='Point-based-sqrt';l{4}='Interval-based';
l{3}='Interval-based method'; l{1}='Point-based method under max mode'; l{2}='Point-based method under sqrt mode';
set(gca,'fontsize',fs);
% set(leg1,'Interpreter','latex');
% title('CH-BnB', 'FontSize', 10);
legend(h1,l,'Location','Northwest','FontSize',fs,'Interpreter','latex');
legend boxoff;
%set(gca,'TickLabelInterpreter','latex','YScale', 'log')
set(gca,'TickLabelInterpreter','latex');
set(gcf,'PaperUnits','inches','PaperPosition',[0 0 13 6])
