function out = fun_inval_maximization(Xset,param,type,dim,eps_F,eps_X,max_iter)
...
Y = fun_inval_wrapper(X,param,type,q);
S = P;
S.h = X;
S.lb = Y(1);
S.ub = Y(2);
...
L = scalar_initialize_lower_bound(S,param,type);
...
iter_ctr_1 = 1;
while ((S_hi.ub-L) > eps_F) && ~is_atomic_hrec(S_hi,eps_X) && (iter_ctr_1 < max_iter)
    ...
    if isequaln(S_hi,S) %compare the two struct
        S_hi = S;
        %Slice S_hi
        [S_L,S_R] = slice_S(S_hi,q,param,type);
        ...
        %Update L
        L = max(L,scalar_eval_left_bound(S_low,param,type));
...
iter_ctr_2 = 1;
while ((S_hi.ub-L) > eps_F) && ~is_atomic_Cov(Cov,eps_X) && (iter_ctr_2 < max_iter)
    %Procedure P(Cov,S_hi,L)
    ...
    if true %isequaln(S_low,S) %compare the two struct
        S_low = S;
        %Slice S_low
        [S_L,S_R] = slice_S(S_low,q,param,type);
...
if  ((S_hi.ub-L) > eps_F) && (length(Cov.dat) > 1) && (~isempty(Cov.dat{1}))
    ...
    while curr_idx <= n_el
        ...
        if ~isempty(Cov.dat{curr_idx})
            %Update L
            L = max(L,scalar_eval_left_bound(Cov.dat{curr_idx},param,type));
...
fprintf('The approximate Lipschitz constant is: %.10f\n',sqrt(S_hi.ub));
